...
export default class EditItemPopOver extends React.PureComponent {
  ...
  handlePortalResize = dimensions => {
    ...
    const {rootLeft, availableHeight, availableWidth, isScrolling} = dimensions
  ...
  render() {
    const {title, children, actions, onClose, isOpen} = this.props

    const {popoverLeft, arrowLeft, maxHeight, isResizing, wantedHeight} = this.state
