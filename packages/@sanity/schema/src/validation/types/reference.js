...
function validateToType(ofType: TypeDef, validateMember: MemberValidator): Array<ValidationResult> {
...
export default {
  validate(typeDef: TypeDef, validateMember: MemberValidator): Array<ValidationResult> {
    ...
    if (Array.isArray(typeDef.to)) {
      ...
      result.push(error('The reference type is missing or having an invalid value for the "to" property', 'schema-reference-type-to-must-be-array'))
    }
    return result
  },
  validateMember(memberTypeDef: TypeDef): Array<ValidationResult> {
    const problems = []
    if (memberTypeDef.type !== 'reference' && memberTypeDef.to) {
      problems.push(error('Cannot overwrite the `to` property on reference types', 'schema-subtype-inheritance'))
    }
    return problems
  }
}
