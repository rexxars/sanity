...
export default function apply(value, patch) {
  ...
  if (!SUPPORTED_PATCH_TYPES.includes(patch.type)) {
    throw new Error(
      `Received patch of unsupported type: "${JSON.stringify(patch.type)}" for primitives. This is most likely a bug.`
    )
  }

  if (patch.path.length > 0) {
    throw new Error(`Cannot apply deep operations on primitive value. Received patch with type "${patch.type}" and path "${patch.path.join('.')}"`)
  }
