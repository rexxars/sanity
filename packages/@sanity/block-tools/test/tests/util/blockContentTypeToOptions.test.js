...
describe('blockContentTypeToOptions', () => {
  it('will give sane default options for default schema', () => {
    ...
    const expected = {
      ...
      enabledSpanDecorators: ['strong', 'em', 'code', 'underline', 'strike-through'],
      enabledBlockStyles: ['normal', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote']
    }
  ...
  it('will give spesific options for custom schema', () => {
    ...
    const expected = {
      ...
      enabledSpanDecorators: ['strong', 'em'],
      enabledBlockStyles: ['normal', 'h1', 'h2']
    }
    assert.deepEqual(blockContentTypeToOptions(blockContentType), expected)
  })
})
