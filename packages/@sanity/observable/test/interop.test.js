...
test('it works with is-observable', t => {
  t.ok(
    ...
    'Expected is-observable to identify SanityObservableMinimal as an Observable'
  )
  t.ok(
...
test('it works with full fledged rx observables', t => {
  const stream = SanityObservable.of('Foo, Bar, SKIP, Baz')
...
test('SanityObservable with flatMap returning an instance of SanityObservableMinimal', t => {
  const stream = SanityObservable.from([1, 2]).flatMap(num => {
...
test('Can be wrapped in an RxObservable', t => {
  ...
  RxObservable.from(
    ...
    .retryWhen(errors =>
      errors.delay(100).scan((count, error) => {
        if (count > 4) {
          throw error
        }
        return count + 1
